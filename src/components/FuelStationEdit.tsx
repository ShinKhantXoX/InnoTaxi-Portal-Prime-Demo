...
import { Fuel, ArrowLeft, Code2, Copy, Check, X, ChevronDown, Info, Pencil, Database, MapPin } from "lucide-react";
import { motion } from "motion/react";
import { fuelStationMockData, type FuelStation } from "./FuelStationList";
...
// ── Update frontend code templates ──
const updateReactCode = `// FuelStationEdit.tsx - PrimeReact + React Router
import { useState, useEffect } from "react";
import { useParams, useNavigate } from "react-router";
import { Fuel, ArrowLeft, Pencil } from "lucide-react";
...
export function FuelStationEdit() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [form, setForm] = useState<FuelStationForm | null>(null);
  ...
  const handleUpdate = async () => {
    const res = await fetch(\`/api/v1/fuel-stations/\${id}\`, {
      method: "PUT",
...
const updateVueCode = `<!-- FuelStationEdit.vue - PrimeVue + Vue Router -->
<template>
  <div class="min-h-full">
    <button @click="router.back()">Back</button>
    <h1>Edit Fuel Station</h1>
    <div class="bg-white rounded-xl border p-6" v-if="form">
...
const handleUpdate = async () => {
  await fetch(\`/api/v1/fuel-stations/\${route.params.id}\`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
const updateAngularCode = `// fuel-station-edit.component.ts - PrimeNG + Angular
...
@Component({
  ...
  template: \`
    <div class="min-h-full" *ngIf="form">
      <button (click)="goBack()">Back</button>
      <h1>Edit Fuel Station</h1>
      <div class="bg-white rounded-xl border p-6">
        <input [(ngModel)]="form.label" />
        <input [(ngModel)]="form.brand" />
        <select [(ngModel)]="form.stationType">
          <option value="GAS">Gas</option>
          <option value="PETROL">Petrol</option>
          <option value="EV">EV</option>
          <option value="GAS_PETROL">Gas + Petrol</option>
...
export class FuelStationEditComponent implements OnInit {
  ...
  handleUpdate(): void {
    const id = this.route.snapshot.paramMap.get('id');
    this.http.put(\`/api/v1/fuel-stations/\${id}\`, this.form).subscribe({
...
const updateBackendCode: Record<BackendLang, string> = {
...
export class FuelStationsController {
  constructor(private readonly svc: FuelStationsService) {}
...
router.put('/api/v1/fuel-stations/:id', async (req: Request, res: Response) => {
  const { id } = req.params;
  const { label, brand, available_fuel_type, station_type, city, township, latitude, longitude, description, status } = req.body;
  const result = await pool.query(
    \`UPDATE fuel_stations SET label=$1, brand=$2, available_fuel_type=$3, station_type=$4, city=$5, township=$6, latitude=$7, longitude=$8, description=$9, status=$10, updated_at=NOW() WHERE id=$11 RETURNING *\`,
    [label, brand, available_fuel_type, station_type, city, township, latitude, longitude, description, status, id]
  );
...
@router.put("/{id}")
async def update_fuel_station(id: int, body: UpdateFuelStation):
    query = """
    UPDATE fuel_stations SET label=:label, brand=:brand,
    available_fuel_type=:available_fuel_type, station_type=:station_type,
    city=:city, township=:township, latitude=:latitude, longitude=:longitude,
    description=:description, status=:status, updated_at=NOW()
    WHERE id=:id RETURNING *
    """
    row = await database.fetch_one(query, {**body.dict(), "id": id})
    if not row:
...
type UpdateFuelStationInput struct {
...
func UpdateFuelStation(c *gin.Context) {
  id, _ := strconv.Atoi(c.Param("id"))
  var input UpdateFuelStationInput
...
import com.innotaxi.dto.UpdateFuelStationDto;
...
public class FuelStationController {
    ...
    public ResponseEntity<Map<String, Object>> update(
        ...
        @RequestBody UpdateFuelStationDto dto
    ) {
        FuelStation fs = service.update(id, dto);
...
class FuelStationController extends Controller
{
    public function update(Request $request, $id)
    {
        ...
        $fs->update($request->only([
            'label', 'brand', 'available_fuel_type', 'station_type',
            'city', 'township', 'latitude', 'longitude', 'description', 'status',
        ]));
        return response()->json(['success' => true, 'data' => $fs->fresh()]);
    }
}`,
...
@api_view(['PUT'])
def fuel_station_update(request, pk):
    try:
        fs = FuelStation.objects.get(pk=pk)
    except FuelStation.DoesNotExist:
        return Response({'success': False, 'message': 'Not found'}, status=404)
    serializer = FuelStationSerializer(fs, data=request.data, partial=True)
    serializer.is_valid(raise_exception=True)
    serializer.save()
    return Response({'success': True, 'data': serializer.data})`,
};

const updateBackendFileConfig: Record<BackendLang, string> = {
...
const fuelStationUpdateSchema = `-- fuel_stations table schema (PostgreSQL)
-- Database: innotaxi
...
SET label = $1, brand = $2, available_fuel_type = $3,
    station_type = $4, city = $5, township = $6,
    latitude = $7, longitude = $8, description = $9,
...
const allFuelTypes = ["Petrol", "Diesel", "CNG", "LPG", "Electric"];
...
export function FuelStationEdit() {
  ...
  useEffect(() => {
    if (item) {
      ...
      setFormFuelTypes([...item.availableFuelType]);
      setFormStationType(item.stationType);
      setFormCity(item.city);
      setFormTownship(item.township);
  ...
  const getCode = () => {
    ...
    if (codeCategory === "database") {
      return fuelStationUpdateSchema;
    }
    return updateBackendCode[backendLang];
  };

  const getFilename = () => {
    if (codeCategory === "frontend") {
      return { react: "FuelStationEdit.tsx", vue: "FuelStationEdit.vue", angular: "fuel-station-edit.component.ts" }[codeFramework];
    }
    if (codeCategory === "database") {
      return "fuel_stations.sql";
    }
    return updateBackendFileConfig[backendLang];
  };
  ...
  return (
    <div>
      {/* ═══ Motion-animated Update Notification ═══ */}
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-[#e2e8f0] overflow-hidden">
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-[13px] text-[#0f172a] font-semibold">Fuel Station Updated</h4>
                <p className="text-[12px] text-[#64748b] mt-0.5">
                  "{formLabel}" has been updated successfully. Redirecting...
                </p>
              </div>
              <button
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#94a3b8]">
                Update fuel station record <span className="text-[#64748b] font-medium">#{item.id}</span>
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#94a3b8] mt-0.5"><span className="text-[#64748b] font-medium">{item.label}</span></p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
          {/* Label */}
          <div>
            <label className="block text-[11px] text-[#94a3b8] uppercase tracking-[0.3px] font-medium mb-1.5">Label</label>
            <input
              type="text"
              value={formLabel}
          ...
          {/* Brand */}
          <div>
            <label className="block text-[11px] text-[#94a3b8] uppercase tracking-[0.3px] font-medium mb-1.5">Brand</label>
            <input
              type="text"
              value={formBrand}
          ...
          {/* City */}
          <div>
            <label className="block text-[11px] text-[#94a3b8] uppercase tracking-[0.3px] font-medium mb-1.5">City</label>
            <input
              type="text"
              value={formCity}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 px-3 py-2 rounded-lg border border-[#e2e8f0] bg-white min-h-[38px]">
              {((): string[] => {
                ...
                return fuelSubTypes[formStationType] || ["Octane 92", "Octane 95", "Standard Diesel", "Premium Diesel"];
              })().map((ft) => {
        ...
        <div className="bg-white rounded-2xl overflow-hidden shadow-2xl border border-[#e2e8f0]">
          ...
          <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0]">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-[13px] text-[#0f172a] font-semibold">Update Fuel Station Code Preview</h3>
                <p className="text-[10px] text-[#94a3b8]">PUT /api/v1/fuel-stations/:id</p>
              </div>
            </div>
            <button
